...
- ReleaseNotes.md の上部に、```バージョン番号```を見出しにして更新内容を英語で書く。
  - オプション名(--xxx)が出るときは、rkmppenc_Options.en.mdへのリンクを作成する。
    オプションのリストとリンクは、rkmppenc_Options.en.md 上部の (## Command line exampleまで)に記載がある
　　例: [--vpp-afs](./rkmppenc_Options.en.md#--vpp-afs-param1value1param2value2)
...
- ブログに記載する文章例を作成する。
  - 更新内容を日本語でHTML形式で記載。(基本的にはgit logのメッセージを使用)
  - 各項目については、見出しとして扱う必要はなく、```<strong>- XXXという機能を追加。</strong>(--option-name)```などのようにすること。
  - ```<br>```は不要。(かわりに通常の改行を用いる)
  - 最後に下記を記載する。
    ```
    <hr size="1" />
    <a href="javascript:void(0);" onclick="javascript:openDownloadUrl('rkmppenc',0);" title="ダウンロード"><strong><u>ダウンロード&gt;&gt;</u></strong></a>
