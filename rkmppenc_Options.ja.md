...
rkmppencの入力方法は下の表のとおり。入力フォーマットをしてしない場合は、拡張子で自動的に判定される。
...
mppencはデフォルトではUTF-8モードで動作するため、AvisynthスクリプトはUTF-8(BOM無し)とする必要がある。
...
入力途中の解像度変更に備えて、入力サーフェスを指定した最大解像度で確保する。
解像度変更後の映像は起動時の出力解像度へリサイズされるため、出力解像度は変化しない。
未指定時は起動時の入力解像度が最大値となる。指定値は起動時の入力解像度以上である必要がある。
...
### --vpp-rff
Repeat Field Flagを反映して、フレームを再構築する。rffによる音ズレ問題が解消できる。[--avsw](#--avsw)と[--avhw](#--avhw)使用時のみ有効。
...
- **パラメータ** ... 基本的にはAviutl版のパラメータをそのまま使用する。
  ...
  - left=&lt;int&gt;          (左)
  - right=&lt;int&gt;         (右)  
    判定に使用する領域から除外する範囲の指定。rkmppencでは、"左"と"右"は4の倍数である必要がある。
...
複数本rkmppencでエンコードをしていて、ひとつのストリームにCPU/GPUの全力を奪われたくないというときのためのオプション。
...
OpenCLをインストールしていない環境やOpenCLが正常に動作しない環境で使用する。
